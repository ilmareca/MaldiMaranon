...
logging.basicConfig(filename='logs/error_generate_bacteria_statistics.log', level=logging.ERROR, 
...
def process_hgugm(base_dir, xml_file, csv_writer):
    try:
        dataset = "HGUGM"
        muestras, maldis = count_extern_ids_and_maldis_hgugm(base_dir, "logs/HGUGM/count_of_externID_HGUGM.log", "logs/HGUGM/count_of_Maldis_HGUGM.log")
...
def process_driams(base_dir, csv_writer):
    try:
        dataset = "DRIAMS"
        muestras = "unknown"
        maldis = count_maldis_driams(base_dir, "logs/DRIAMS/count_of_Maldis_DRIAMS.log")
...
def process_rki(base_dir, xml_file, csv_writer):
    try:
        dataset = "RKI"
        muestras, maldis = count_extern_ids_and_maldis_rki(base_dir, "logs/RKI/count_of_externID_RKI.log", "logs/RKI/count_of_Maldis_RKI.log")
        genus, species = count_genus_species(xml_file)
